...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    def go(ar: Array[A]): Boolean = {
      if (gt(ar(0), ar(1)) && ar.length == 2) true else if (gt(ar(0), ar(1)) && go(ar.tail)) true else false
    }

    if (as.length < 2) true else go(as)
  }

  def main(args: Array[String]): Unit = {
    val array = Array(1, 2, 3, 4, 3)
    println(isSorted(array, (i: Int, j: Int) => i <= j))
  }
